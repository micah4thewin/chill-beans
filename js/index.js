          ...
          <div class="d-flex justify-content-between align-items-center">
            <div class="btn-group" role="group">
              ${item.sizes
                .map(
                  (size, sizeIndex) =>
                    `<button id="size-btn-${index}-${sizeIndex}" type="button" class="btn btn-secondary" data-item='${JSON.stringify(
                      item
  ...
  const populateCardSection = async () => {
    ...
    cardSection.addEventListener("click", (event) => {
      if (event.target.tagName === "BUTTON") {
        ...
        document.getElementById(`price-${currentIndex}`).innerHTML = `<small class="text-muted fs-3">${item.sizes[newSizeIndex].price}</small>`;
      }
    });
